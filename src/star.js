class Star{
    ...
    update(coordinate, time){
        ...
        this.altitude = this.toDegrees(Math.asin((Math.sin(this.toRadians(latitude)) * Math.sin(this.toRadians(this.declination)) + (Math.cos(this.toRadians(latitude)) * Math.cos(this.toRadians(this.declination)) * Math.cos(this.toRadians(LHA / 24.9) * 360)))))
